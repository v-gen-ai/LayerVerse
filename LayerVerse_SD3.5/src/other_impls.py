...
# adds attention_with_cross_attn_map for the automatic editing mask.
...
def attention_with_cross_attn_map(q, k, v, heads, context_len, t5_start):
    """Compute attention and extract image-to-CLIP and image-to-T5 cross-attention.

    The joint sequence is ``[context | image]``.  ``context_len`` tokens come
    from text ``[registers | CLIP(77) | T5(N)]`` and the rest are image patches.

    CLIP occupies columns ``[t5_start - 77 : t5_start]`` and T5 occupies
    ``[t5_start : context_len]``.

    Returns ``(attn_output, clip_cross_attn, t5_cross_attn)`` where both
    cross-attention tensors have shape ``(B, img_tokens, num_text_tokens)``
    averaged over heads.
    """
    b, _, dim_head = q.shape
    dim_head //= heads
    ...
    clip_cross_attn = attn_weight[:, :, context_len:, t5_start - 77:t5_start]
    clip_cross_attn = clip_cross_attn.mean(dim=1).detach()  # (B, P, 77)

    t5_cross_attn = attn_weight[:, :, context_len:, t5_start:context_len]
    t5_cross_attn = t5_cross_attn.mean(dim=1).detach()  # (B, P, N_t5)

    out = torch.matmul(attn_weight, v)
    return out.transpose(1, 2).reshape(b, -1, heads * dim_head), clip_cross_attn, t5_cross_attn
