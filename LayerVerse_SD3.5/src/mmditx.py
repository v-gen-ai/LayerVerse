...
# adds KV-injection and cross-attention map extraction, driven by the `info` dict.
...
from other_impls import Mlp, attention, attention_with_cross_attn_map
...
def _apply_kv_injection(info, qkv, key_suffix=""):
    """LayerVerse KV-injection for one image QKV tuple.

    During inversion the image K/V of every active block are cached per timestep.
    During editing they replace the target K/V:
    - global_blocks:   everywhere (structural anchors);
    - selected_blocks: outside the editing mask only (background protection).
    A block listed in both is global; blocks in neither are left untouched.
    """
    block_id = info.get("block_id", 0)
    in_selected = block_id in info.get("selected_blocks", [])
    in_global = block_id in info.get("global_blocks", [])

    if not in_selected and not in_global:
        return qkv

    t_key = f"{info.get('timestep', 0):.4f}"
    feature_key_k = f"{t_key}_{block_id}_K{key_suffix}"
    feature_key_v = f"{t_key}_{block_id}_V{key_suffix}"

    if info.get("inverse"):
        info["features"][feature_key_k] = qkv[1].cpu()
        info["features"][feature_key_v] = qkv[2].cpu()
    elif feature_key_k in info["features"]:
        source_k = info["features"][feature_key_k].to(qkv[1].device, dtype=qkv[1].dtype)
        source_v = info["features"][feature_key_v].to(qkv[2].device, dtype=qkv[2].dtype)
        if not in_global and "edit_map_indices" in info and info.get("use_mask_injection"):
            edit_idx = info["edit_map_indices"]
            source_k[:, edit_idx, :] = qkv[1][:, edit_idx, :]
            source_v[:, edit_idx, :] = qkv[2][:, edit_idx, :]
        qkv = (qkv[0], source_k, source_v)

    return qkv


def block_mixing(context, x, context_block, x_block, c, info=None):
    ...
    if info is not None and info.get("use_kv_injection"):
        x_qkv = _apply_kv_injection(info, x_qkv, key_suffix="")
        if x_block.x_block_self_attn:
            x_qkv2 = _apply_kv_injection(info, x_qkv2, key_suffix="_self")
    ...
    _extract = (
        info is not None
        and info.get("extract_attn_maps")
        and info.get("block_id") in info.get("extract_blocks", [])
    )
    if _extract:
        ctx_len = context_qkv[0].shape[1]
        attn, clip_map, t5_map = attention_with_cross_attn_map(
            q, k, v, x_block.attn.num_heads, ctx_len, info["t5_token_offset"],
        )
        info["clip_attn_maps"][info["block_id"]] = clip_map
        info["t5_attn_maps"][info["block_id"]] = t5_map
    else:
...
class JointBlock(nn.Module):
    ...
    def forward(self, context, x, c, info=None):
        return block_mixing(
            context, x, self.context_block, self.x_block, c, info=info
        )
...
class MMDiTX(nn.Module):
    ...
    def forward_core_with_concat(
        ...
        for i, block in enumerate(self.joint_blocks):
            ...
            if info is not None:
                info["block_id"] = i
            context, x = block(context, x, c=c_mod, info=info)
    ...
    def forward(
        ...
        info: Optional dict for KV-injection (features, selected_blocks, etc.)
        """
        hw = x.shape[-2:]
        ...
        x = self.forward_core_with_concat(x, c, context, skip_layers, controlnet_hidden_states, info)
